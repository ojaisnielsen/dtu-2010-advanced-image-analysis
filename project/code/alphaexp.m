...
a = -beta * exp(-abs(current(x) - current(y))/100);%beta);%(current(x) == current(y));
b = -beta * exp(-abs(current(x) - k)/100);%beta);%(current(x) == k);
c = -beta * exp(-abs(current(y) - k)/100);%beta);%(current(y) == k);
d = -beta * ones(size(x));
